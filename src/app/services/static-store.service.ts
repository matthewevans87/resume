...
export class StaticStoreService {
    ...
    public getResume(): Observable<Resume> {
        if (null == this.resume$) {
            ...
            this.httpClient.get<Resume>('assets/resume.json')
                .pipe(
                    tap(resume => console.log(resume))
                )
    ...
    public getTermsDictionary(): Observable<{ [termName: string]: Term }> {
        if (null == this.termsDictionary$) {
            this.termsDictionary$ = new ReplaySubject<{ [termName: string]: Term }>(1);
            this.httpClient.get<Term[]>('assets/terms.json')
                .pipe(
                    map(terms => {
